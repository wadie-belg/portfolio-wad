...
├── data/
│   ├── content.json        ← قاعدة البيانات الديناميكية (577 سطر)
│   ├── lang/
...
## 🗄️ 4. data/content.json — قاعدة البيانات (The Vault)

### ما هو؟
ملف JSON (577 سطر، 14 KB) يحتوي على **جميع البيانات** — مثل **خزنة المعلومات** الآمنة.

### هيكل البيانات:
- **`personal`**: الاسم، العنوان، الهاتف، الإيميل، تاريخ الميلاد، الجنسية، جواز السفر
- **`about`**: نبذة بالإنجليزية والعربية
- **`education`**: 6 درجات/شهادات (كل منها: سنة، درجة، مؤسسة)
- **`experience`**: 3 فئات × عناصر (كل عنصر: فترة، دور، مؤسسة)
- **`conferences`**: 10+ مؤتمر (كل واحد: سنة، عنوان، منظمة)
- **`skills`**: لغات + مهارات تقنية
- **`extra.publications`**: 1 منشور
- **`extra.hobbies`**: 5 هوايات
- **`achievements`**: 6 بطاقات إنجاز (أيقونة، عنوان، وصف، مستوى)

> **التشبيه**: مثل **المكتبة** — كل المعلومات محفوظة هنا بترتيب. يمكنك تغيير أي معلومة بدون تعديل HTML أو CSS.

---

## 🌐 5. Language Files — ملفات الترجمة
...
### 1. **معمارية البيانات المنفصلة (Separation of Concerns)**
البيانات في JSON منفصلة عن العرض (HTML/CSS) عن التفاعل (JS).
> **التشبيه**: مثل مطعم — المطبخ (JSON) يُحضّر، الطاولات (HTML) تعرض، النادل (JS) يخدم. كل فريق يعمل لوحده.
...
> **التشبيه النهائي**: مثل **سيارة فاخرة** — الهيكل (HTML) + التصميم الخارجي (CSS) + المحرك والتحكم (JS) + الوقود (JSON). كل قطعة تعمل معاً بسلاسة مثل الساعة السويسرية.
